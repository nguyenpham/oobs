...
## EPD strings
Instead of using "magic" keys such as hash keys, we use EPD (Extended Position Description) strings to represent/store chess positions. An EPD is a short form of a FEN when the last two numbers halfmove clock and fullmove are tripped. Those EPD should not have any operations.
